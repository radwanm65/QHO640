...
const sessDb = new Database("students.db");
...
app.get("/song/:id", (req, res) => {
  const { id } = req.params;
  console.log("Buying Song Number (Server Side) : ", id);
  const song = db.prepare(`SELECT * FROM wadsongs WHERE id = ?`).get(id);
...
app.get("/song/:id", (req, res) => {
  const { id } = req.params;
  console.log("Get Song Number (Server Side) : ", id);
  const song = db.prepare(`SELECT * FROM wadsongs WHERE id = ?`).get(id);
